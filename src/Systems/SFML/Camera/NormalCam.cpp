...
namespace fengin::systems::SFMLSystems {
    ...
    void NormalCam::renderWindow(Entity &cam)
    {
    ...
    void NormalCam::drawCamCrosshair(components::Camera &cam, sf::RenderWindow *window)
    {
    ...
    void NormalCam::renderCam(Entity &entity, components::Camera &cam, components::World &world)
    {
    ...
    void NormalCam::sortGameObjects() {
        layout.clear();
        ...
        for (auto &go: gameObjects)
        {
            ...
            layout.insert(std::pair<int, Entity *>(transform.position.z, &entity));
        }
    }

    void NormalCam::run(float e) {
        static float t = 0.0;
        t+=e;
        switch (phase)
        {
